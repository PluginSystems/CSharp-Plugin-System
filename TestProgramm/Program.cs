...
namespace TestProgramm
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            {
                stats.ForEach(testcase => { testcase.RunTestFully(cycle); });

                stats.ForEach(finishedTest =>
                {
                    var streamWriter = new StreamWriter("./results_" + cycle + "_" + finishedTest.GetName() +
                                                        "_nanoseconds_" + currentTimeMillis + ".csv");
